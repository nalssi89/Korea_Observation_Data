...
def process_rows(mode: str) -> list[list[str]]:
    return [
        ...
        ["5", "기본 근거 선택", f"ONI {mode} phase 월별·계절연도 합성표와 영향 계절연도 표를 먼저 확인"],
...
def criteria_layer_rows(phase: str) -> list[list[str]]:
    threshold = "+0.5도 이상" if phase == "엘니뇨" else "-0.5도 이하"
    extreme = "첫 ONI 최댓값" if phase == "엘니뇨" else "첫 ONI 최솟값"
    continue_year = "1987년 엘니뇨" if phase == "엘니뇨" else "2022년 라니냐"
    return [
        [
            "1. ENSO 상태 판정",
            f"ONI 3개월 겹침 계절평균이 {threshold}이고 같은 부호가 5개 이상 연속될 때 공식 episode로 확정",
            "확정 전에는 발달 가능성 또는 감시 단계로 표현. RONI는 보조 민감도 확인",
        ],
        [
            "2. 조기 소통",
            "5개 연속 계절을 기다리면 사후 확인이 되므로 주별·월별 SST와 대기 결합 신호는 별도 감시",
            "'발생' 또는 '확정' 대신 현재 단계와 불확실성을 함께 설명",
        ],
        [
            "3. 영향 판단",
            "달력년이 아니라 질문 대상 월·계절 자체를 기준으로 표본 선택",
            f"여름·겨울 질문은 JJA/DJF 영향 계절연도 표를 우선 사용하고 {continue_year}처럼 지속 episode도 포함",
        ],
        [
            "4. 발달기·소멸기",
            f"공식 episode 안에서 시작부터 {extreme}까지를 발달기, 이후 종료까지를 소멸기로 구분",
            "발달/소멸 라벨은 해당 월·계절에 붙이고 그해 전체에 붙이지 않음",
        ],
        [
            "5. 달력년 약칭",
            f"'{continue_year} 지속해' 같은 표현은 대외 설명용 약칭으로만 사용",
            "달력년 약칭을 기온·강수·태풍 영향의 직접 근거로 쓰지 않음",
        ],
    ]


def question_priority_rows() -> list[list[str]]:
    return [
...
def wording_rows(phase: str) -> list[list[str]]:
    threshold = "+0.5도 이상" if phase == "엘니뇨" else "-0.5도 이하"
    return [
        [
            f"ONI 3개월 평균이 {threshold}이나 지속성 미확인",
            f"{phase} 발달 가능성이 커졌고 감시 중이다",
            f"{phase}가 발생했다",
        ],
        [
            "5개 이상 연속 계절을 충족",
            f"공식 ONI 기준상 {phase} 상태다",
            "한반도 기온·강수·태풍 영향까지 확정됐다",
        ],
        [
            "여름·겨울 영향 질문",
            "해당 JJA/DJF 계절이 ONI phase였던 과거 계절연도를 우선 비교한다",
            "episode 시작연도만으로 영향 표본을 고른다",
        ],
        [
            "발달기·소멸기 질문",
            "해당 계절은 발달기 또는 소멸기 표본에 속한다고 표현한다",
            "그해 전체가 발달기 또는 소멸기라고 표현한다",
        ],
    ]
    ...
    sections = [
        ...
        Section(
            ...
            [
                make_callout("공식 선언은 늦고, 감시는 빠르게 한다."),
                make_paragraph(
                    "라니냐는 Niño3.4 해역의 ONI 3개월 겹침 계절평균이 -0.5도 이하이고 이 상태가 5개 이상 연속되는지를 확인해야 합니다. 따라서 '라니냐 확정'과 '라니냐 쪽으로 기울고 있음'을 분리해 말합니다."
                ),
                make_table(
                    ["상황", "권장 표현", "피해야 할 표현"],
                    [
